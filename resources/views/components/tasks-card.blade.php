            ...
            <div class="row">
                ...
                @if (isAdminOrHasAllDataAccess() && !isset($viewAssigned))
                    @if (explode('_', $id)[0] != 'client' && explode('_', $id)[0] != 'user')
                        <div class="col-md-4 mb-3">
                            <select class="form-control " id="tasks_user_filter" name="user_ids[]" multiple="multiple"
                                data-placeholder="<?= get_label('select_users', 'Select Users') ?>">

                            </select>
                        </div>
                        <div class="col-md-4 mb-3">
                            <select class="form-control " id="tasks_client_filter" name="client_ids[]" multiple="multiple"
                ...
                <table id="task_table" data-toggle="table" data-loading-template="loadingTemplate"
                    ...
                    <thead>
                        <tr>
                            ...
                            <th data-field="billing_type"
                                data-visible="{{ in_array('billing_type', $visibleColumns) || empty($visibleColumns) ? 'true' : 'false' }}">
                                {{ get_label('billing_type', 'Billing type') }}
                            </th>

...
class MeetingsController extends Controller
{
            ...
            ->through(function ($meeting) use ($canEdit, $canDelete, $canCreate, $currentDateTime) {
                ...
                return [
                    ...
                    'title' => ($status == 'Ongoing')
                        ? '<a href="/master-panel/meetings/join/' . $meeting->id . '" target="_blank" class="text-primary" title="Join">' .
    ...
    public function update(Request $request)
    {
        ...
        $end_date = $request->input('end_date');
        $end_time = $request->input('end_time');
        $formFields['start_date_time'] = format_date(
            $start_date . ' ' . $start_time,
            false,
            app('php_date_format') . ' H:i:s',
            'Y-m-d H:i:s',
            false
        );

        $formFields['end_date_time'] = format_date(
            $end_date . ' ' . $end_time,
            false,
            app('php_date_format') . ' H:i:s',
            'Y-m-d H:i:s',

    ...
    public function list()
    {
        ...
        $plans = $plans->map(function ($plan) {
            ...
            return [
                ...
                'yearly_price' => ($plan->plan_type != 'free' && $plan->yearly_price == 0.0) ? '-' : format_currency($plan->yearly_price),
                'yearly_discounted_price' => ($plan->plan_type != 'free' && $plan->yearly_discounted_price == 0.0) ? '-' : format_currency($plan->yearly_discounted_price),
                'lifetime_price' => ($plan->plan_type != 'free' && $plan->lifetime_price == 0.0) ? '-' : format_currency($plan->lifetime_price),
                'lifetime_discounted_price' => ($plan->plan_type != 'free' && $plan->lifetime_discounted_price == 0.0) ? '-' : format_currency($plan->lifetime_discounted_price),

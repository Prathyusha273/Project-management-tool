        ...
        switch ($column) {
            case 'default_view':
                ...
                if ($table == 'projects') {
                        ...
                        ? ($result->default_view == 'list' ? 'list'
                            : ($result->default_view == 'kanban_view' ? 'kanban_view'
                                : ($result->default_view == 'grid' ? 'grid'
                                    : ($result->default_view == 'calendar-view' ? 'calendar-view'
                                        : 'projects'))))
                        : 'projects';
                    ...
                    return $result && $result->default_view
                        ? ($result->default_view == 'list' ? 'list'
                            : ($result->default_view == 'kanban' ? 'kanban'
                                : 'leads'))
                        : 'leads';
...
function mapUserFormatToPhpFormat($userFormat)
{
    // Common transformations
    $map = [
        'M' => 'm', // Convert month name format to numeric month format if needed
        // Add other mappings as necessary
    ];

    foreach ($map as $from => $to) {
        $userFormat = str_replace($from, $to, $userFormat);
    }
